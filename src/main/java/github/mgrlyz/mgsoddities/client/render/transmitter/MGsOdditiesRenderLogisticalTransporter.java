...
public class MGsOdditiesRenderLogisticalTransporter extends RenderTransmitterBase<MGsOdditiesTileEntityLogisticalTransporterBase> {
    ...
    private final MGsOdditiesRenderLogisticalTransporter.LazyItemRenderer itemRenderer = new MGsOdditiesRenderLogisticalTransporter.LazyItemRenderer();

    public MGsOdditiesRenderLogisticalTransporter(BlockEntityRendererProvider.Context context) {
        super(context);
        modelBox = new ModelTransporterBox(context.getModelSet());
    }

    @Override
    protected void render(@NotNull MGsOdditiesTileEntityLogisticalTransporterBase tile, float partialTick, @NotNull PoseStack matrix, @NotNull MultiBufferSource renderer, int light, int overlayLight, @NotNull ProfilerFiller profiler) {
    ...
    @Override
    protected boolean shouldRenderTransmitter(@NotNull MGsOdditiesTileEntityLogisticalTransporterBase tile, @NotNull Vec3 camera) {
    ...
    private Collection<TransporterStack> getReducedTransit(Collection<TransporterStack> inTransit) {
        ...
        Set<MGsOdditiesRenderLogisticalTransporter.TransportInformation> information = new ObjectOpenHashSet<>();
        for (TransporterStack stack : inTransit) {
            if (stack != null && !stack.itemStack.isEmpty() && information.add(new MGsOdditiesRenderLogisticalTransporter.TransportInformation(stack))) {
    ...
    private static class TransportInformation {
        ...
        public boolean equals(Object obj) {
            ...
            return obj instanceof MGsOdditiesRenderLogisticalTransporter.TransportInformation other && progress == other.progress && color == other.color && item.equals(other.item);
        }
    }
